...
import { passkeyClient } from "@/lib/auth-client";
...
export function AdminPasskeys() {
  ...
  const updatePasskey = useMutation({
    mutationFn: ({ id, name }: { id: string; name: string }) =>
      passkeyClient.updatePasskey({ id, name, fetchOptions: { throw: true } }),
    onSuccess() {
      router.invalidate();
    },
  });
  ...
  const deletePasskey = useMutation({
    mutationFn: (id: string) =>
      passkeyClient.deletePasskey({ id, fetchOptions: { throw: true } }),
    onSuccess() {
      router.invalidate();
    },
  });
  ...
  const registerPasskey = useMutation({
    mutationFn: (name: string) => {
      return passkeyClient.addPasskey({
        name: name || undefined,
