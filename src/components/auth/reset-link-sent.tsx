...
export function ResetLinkSent({ className }: ResetLinkSentProps) {
  ...
  const [email, setEmail] = useState((isHydrated && sessionStorage.getItem(RESET_LINK_SENT_STORAGE_KEY)) || "");
  ...
  return (
    <Card className={cn("w-full max-w-sm", className)}>
      <CardHeader>
        <CardTitle className="text-xl font-semibold">{localization.auth.checkYourEmailTitle}</CardTitle>
      </CardHeader>
