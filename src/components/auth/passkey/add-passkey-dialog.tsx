...
export function AddPasskeyDialog({ open, onOpenChange }: AddPasskeyDialogProps) {
  ...
  const { authenticatorAttachment, localization: passkeyLocalization } = useAuthPlugin(passkeyPlugin);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent>
        {needsFreshSession ? (
          <>
            <DialogHeader>
              <DialogTitle className="sr-only">{localization.settings.freshSessionTitle}</DialogTitle>
            </DialogHeader>
            <FreshSessionPrompt
              onFresh={() => {
              ...
              <form.AppField name="name">
                {(field) => (
                  <Field data-invalid={addPasskey.isError}>
                    ...
                    />
                    {addPasskey.error && (
                      <FieldError>{addPasskey.error.error?.message ?? addPasskey.error.message}</FieldError>
                    )}
                  </Field>
                )}
              </form.AppField>
