...
export function AccountSettings({ className, ...props }: AccountSettingsProps & ComponentProps<"div">) {
  ...
  const ChangeEmailOverride = plugins.find((plugin) => plugin.cardOverrides?.account?.changeEmail)?.cardOverrides
    ?.account?.changeEmail;
  ...
  return (
    <div className={cn("flex w-full flex-col gap-4 md:gap-6", className)} {...props}>
      ...
      {plugins.flatMap(
        (plugin) => plugin.accountCards?.map((Card, index) => <Card key={`${plugin.id}-${index.toString()}`} />) ?? [],
      )}
    </div>
  );
}
