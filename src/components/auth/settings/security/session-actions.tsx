  ...
  return (
    <>
      <div className="flex flex-wrap justify-end gap-2 border-t p-4">
        ...
        <Button disabled={isPending} onClick={() => setAction("all")} size="sm" type="button" variant="destructive">
          {localization.settings.signOutEverywhere}
        </Button>
      </div>

      <AlertDialog open={action !== null} onOpenChange={(open) => !open && !isPending && setAction(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogTitle>
              {isEverywhere ? localization.settings.signOutEverywhere : localization.settings.signOutOtherDevices}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {isEverywhere
