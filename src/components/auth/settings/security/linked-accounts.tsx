...
export function LinkedAccounts({ className }: LinkedAccountsProps) {
  const { authClient, allowUnlinkingAllAccounts, localization, multipleAccountsPerProvider, socialProviders } =
    useAuth();
  ...
  const allRows = [
    ...(linkedAccounts?.map((account) => ({
      ...
      account,
      provider:
        socialProviders?.find((provider) => getProviderId(provider) === account.providerId) ?? account.providerId,
    })) ?? []),
            ...
            {isPending
              ...
              : allRows.map((row, index) => (
                  <Fragment key={row.key}>
                    {index > 0 && <ItemSeparator />}
                    <LinkedAccount account={row.account} canUnlink={canUnlink} provider={row.provider} />
                  </Fragment>
                ))}
          </ItemGroup>
        </CardContent>
