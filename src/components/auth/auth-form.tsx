...
export function focusFirstInvalidAuthFormControl(form: HTMLFormElement) {
  requestAnimationFrame(() => {
    form.querySelector<HTMLElement>('[aria-invalid="true"]:not([disabled]), :invalid:not([disabled])')?.focus();
  });
}
...
function AuthFormRoot({ children, onBeforeSubmit, ...props }: AuthFormRootProps) {
  ...
  return (
    <form {...props} onInvalid={(event) => focusFirstInvalidAuthFormControl(event.currentTarget)} onSubmit={submit}>
      {children}
    </form>
  );
}
...
export function isAuthFormFieldInvalid({ isTouched, isValid }: { isTouched: boolean; isValid: boolean }) {
  return isTouched && !isValid;
}

export function getAuthAdditionalFieldValidators(field: AdditionalFieldConfig, requiredMessage: string) {
  return {
    ...
    onChangeAsync: field.validate
      ? ({ value }: { value: AdditionalFieldFormValue }) => validateAdditionalFieldValue(field, value)
      : undefined,
