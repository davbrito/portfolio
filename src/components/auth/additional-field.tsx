    ...
    return (
      <Field data-invalid={isInvalid}>
        ...
        <Combobox
          ...
          disabled={isPending || field.readOnly}
        >
          <ComboboxInput placeholder={field.placeholder} id={name} onBlur={onBlur} aria-invalid={isInvalid} />
...
function InputField({ name, field, value, onBlur, onChange, isInvalid, errors, isPending }: AdditionalFieldProps) {
 ...
 */
function SliderField({ name, field, value, onBlur, onChange, isInvalid, errors, isPending }: AdditionalFieldProps) {
  const maxFractionDigits = field.formatOptions?.maximumFractionDigits;
  ...
  return (
    <Field data-invalid={isInvalid}>
      <div className="flex items-center justify-between gap-2">
        <FieldLabel htmlFor={name}>{field.label}</FieldLabel>
        <span className="text-muted-foreground text-sm tabular-nums">{formatter.format(numericValue)}</span>
      </div>
      ...
      <Slider
        ...
        value={[numericValue]}
        onBlur={onBlur}
        onValueChange={(nextValue) => onChange((Array.isArray(nextValue) ? nextValue[0] : nextValue) ?? min)}
        min={min}
        max={max}
        step={step}
        disabled={isPending || field.readOnly}
 ...
 */
function DateInput({ name, field, value, onBlur, onChange, isInvalid, errors, isPending }: AdditionalFieldProps) {
  const { localization } = useAuth();
  const inputType = resolveInputType(field);
